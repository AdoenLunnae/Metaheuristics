 ...
 * @param[in] instance Instance of the CriticalNode
 */
void runARandomSearchExperiment(vector<double>& results, CriticalNodeInstance& instance)
{

    CriticalNodeSolution currentSolution(instance);
    RandomSolutionGenerator r;
    Timer t;
    ...
    while (t.elapsed_time(Timer::VIRTUAL) <= MAX_SECONDS_PER_RUN && numSolutions < MAX_SOLUTIONS_PER_RUN) {
        currentSolution = r.generate(instance);
        double currentFitness = CriticalNodeEvaluator::getFitness(instance, currentSolution);
...
void runExperiments(vector<vector<vector<double>*>*>& results, char** mainArgs, unsigned int numMainArgs)
{
    ...
    for (unsigned int iInstance = 1; iInstance < numMainArgs; iInstance += 2) {
        ...
        instance.setMaxDeleted(atoi(mainArgs[iInstance + 1]));
        instance.readAdjacencyList(instanceName);
...
void computeMeans(vector<vector<double>*>& results, vector<currentBestPair>& means, unsigned int overallLongestRun)
{
    ...
    previousMean.best = numeric_limits<double>::min();
...
int main(int argc, char** argv)
{
    ...
    if (argc < 3 || (argc % 2) != 1) {
        cout << argv[0] << " (<problem instance files> <deletedVertices>)+" << endl;
        return 0;
    }
