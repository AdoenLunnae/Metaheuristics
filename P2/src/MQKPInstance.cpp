...
double MQKPInstance::getMaxCapacityViolation(MQKPSolution& solution)
{
    ...
    for (int i = 0; i < this->_numObjs; i++) {
        ...
        if (solution.whereIsObject(i) > 0) {
            sumWeights[solution.whereIsObject(i)] = sumWeights[solution.whereIsObject(i)] + getWeight(i);
        }
    }
    ...
    int indexKnapsack)
{
    ...
    if (solution.whereIsObject(indexObject) != 0) {
        ...
        for (int indexOther = 0; indexOther < getNumObjs(); ++indexOther) {
            if ((indexOther != indexObject) && (solution.whereIsObject(indexOther) == solution.whereIsObject(indexObject)))
                deltaSumProfits -= getProfit(indexObject, indexOther);
        }
    }
    /* 
    ...
    if (indexKnapsack != 0) {
        deltaSumProfits += getProfit(indexObject);
        for (int indexOther = 0; indexOther < getNumObjs(); ++indexOther) {
            if ((indexOther != indexObject) && (solution.whereIsObject(indexOther) == indexKnapsack))
                deltaSumProfits += getProfit(indexObject, indexOther);
        }
    }
    ...
    int indexObject, int indexKnapsack)
{
	 ...
	 */
    int currentKnapsack = solution.whereIsObject(indexObject);
    double currentViolation, newViolation;

    solution.putObjectIn(indexObject, indexKnapsack);
    newViolation = getMaxCapacityViolation(solution);
    solution.putObjectIn(indexObject, currentKnapsack);

    currentViolation = getMaxCapacityViolation(solution);
